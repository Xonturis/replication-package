...
func handleTotalSongs(socket *socket.Socket) events.Listener {
	return func(a ...any) {
		logger := utils.GetLogger()

		logger.Debug("sdfqsdfsdfs")
		ctx := context.Background()
...
func AudioDataForRecognition(strAudio string) []byte {
	// Parse WAV File
	logger := utils.GetLogger()
	ctx := context.Background()

	var recData models.RecordData
	if err := json.Unmarshal([]byte(strAudio), &recData); err != nil {
		err := xerrors.New(err)
		logger.ErrorContext(ctx, "Failed to unmarshal record data.", slog.Any("error", err))
		return []byte{}
	}
...
func handleSaaSFind(socket *socket.Socket) events.Listener {
	return func(a ...any) {
		jsonData := FingerprintDataForRecognition((a[0].(string)))
		if len(jsonData) == 0 {
			return
		}
...
func FingerprintDataForRecognition(strFingerprint string) []byte {
	logger := utils.GetLogger()
	ctx := context.Background()
	...
	if err := json.Unmarshal([]byte(strFingerprint), &data); err != nil {
		err := xerrors.New(err)
...
func handleNewFingerprint(socket *socket.Socket) events.Listener {
	return func(a ...any) {
